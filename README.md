...
Moodtips 是一个已经部署并投入真实使用的情绪饮品推荐系统。用户选择当下状态、口味、冷热、价格和咖啡因偏好后，系统会从饮品库中筛选候选 SKU，并给出推荐理由、图片来源可信度和后续操作入口。

正式使用入口：[https://moodtips.oppenchow.online/app/](https://moodtips.oppenchow.online/app/)

如果只是日常使用 Moodtips，不需要把项目部署到本地，直接访问线上网站即可。本仓库开源主要用于学习交流、课程展示、技术复现和二次开发。

也欢迎大家共同维护饮品库：补充缺失的饮品图片、上传新饮品、协助校对品牌、价格、口味标签和图片来源，让推荐结果越来越完整、准确、好用。
...
本地启动仅面向希望学习源码、复现系统、改造功能或参与开发的人。
...
本仓库包含学习、复现和二次开发 Moodtips 所需的代码、数据库和前台素材。课程报告、课堂展示页、历史压缩包、缓存文件和旧版本操作文件没有放入此开源目录。
